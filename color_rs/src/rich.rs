...
impl Rich {
    pub fn set_formatter(format: &str) {
        let mut f = FORMATTER
            .write()
            .expect("Failed to lock formatter for writing");
        *f = format.to_string();
    }
    ...
    pub fn parse_colors(mut input: String, clean: bool) -> String {
        ...
        for (tag, content) in color_tags {
            ...
            let color_code =
                Self::value_process(color_value, is_background).unwrap_or_else(|| String::new());
        ...
        for (tag, content) in tags {
            if content == "TIMESTAMP" {
                ...
                let format = FORMATTER
                    .read()
                    .expect("Failed to lock formatter for reading");
