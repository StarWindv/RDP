...
use crate::modules::ast::{
    AndOrOperator, AstNode, CaseClause, CommandSeparator, ParameterOperation, RedirectType,
};
...
impl SsaIrGenerator {
    ...
    fn generate_node(&mut self, node: AstNode) -> ValueId {
        match node {
            ...
            // Simple Commands
            // ============================================
            AstNode::SimpleCommand { name, args, .. } => self.generate_simple_command(&name, &args),

            AstNode::Assignment { name, value, .. } => self.generate_assignment(&name, &value),

            // ============================================
            // Pipelines and Lists
            // ============================================
            AstNode::Pipeline { commands, .. } => self.generate_pipeline(commands),
            ...
            AstNode::LogicalAnd { left, right, .. } => self.generate_logical_and(*left, *right),

            AstNode::LogicalOr { left, right, .. } => self.generate_logical_or(*left, *right),
            ...
            // Compound Commands
            // ============================================
            AstNode::CompoundCommand { commands, .. } => self.generate_compound_command(commands),

            AstNode::Subshell { commands, .. } => self.generate_subshell(commands),
            ...
            AstNode::Background { command, .. } => self.generate_background(*command),
            ...
            // ============================================
            AstNode::Export { variables, .. } => self.generate_export_command(&variables),

            AstNode::Unset { variables, .. } => self.generate_unset_command(&variables),
            ...
            AstNode::Error { message, token } => self.generate_error(&message, token),
        }
    }
    ...
    fn generate_and_or_list(
        &mut self,
        commands: Vec<Box<AstNode>>,
        operators: &[AndOrOperator],
    ) -> ValueId {
        if commands.is_empty() {
            return self.generate_null_command();
        }
        ...
        for (i, cmd) in commands.iter().skip(1).enumerate() {
            ...
            match operator {
                AndOrOperator::AndIf => {
                    ...
                    self.add_instruction(Instruction::Branch(
                        cond, eval_block, // If success (status == 0), evaluate next
                ...
                AndOrOperator::OrIf => {
                    ...
                    self.add_instruction(Instruction::Branch(
                        cond, eval_block, // If failure (status != 0), evaluate next
            ...
            self.add_instruction(Instruction::Phi(
                vec![(eval_block, cmd_status), (skip_block, current_status)],
                phi_result,
            ));
    ...
    fn generate_command_list(
        &mut self,
        commands: Vec<Box<AstNode>>,
        separators: &[CommandSeparator],
    ) -> ValueId {
        if commands.is_empty() {
            return self.generate_null_command();
        }
    ...
    fn generate_if_statement(
        ...
        let cond = self.create_value(ValueType::Boolean);
        let zero_const = self.create_const_int(0);
        self.add_instruction(Instruction::Cmp(cond_status, zero_const, CmpOp::Eq, cond));
        self.add_instruction(Instruction::Branch(cond, then_block, else_block));
        ...
        let result = self.create_value(ValueType::ExitStatus);
        self.add_instruction(Instruction::Phi(
            vec![(then_block, then_result), (else_block, else_result)],
            result,
        ));
    ...
    fn generate_case_statement(&mut self, word: &str, cases: Vec<CaseClause>) -> ValueId {
        ...
        for (i, (case_block, next_block, case)) in case_blocks.iter().enumerate() {
            ...
            self.add_instruction(Instruction::Branch(combined_match, body_block, *next_block));

            // Body block: execute case commands
            self.set_current_block(body_block);
            let _body_result =
                self.generate_compound_command(case.body.iter().map(|c| c.clone()).collect());
    ...
    fn generate_until_loop(&mut self, condition: AstNode, body: Vec<Box<AstNode>>) -> ValueId {
        ...
        let cond = self.create_value(ValueType::Boolean);
        let zero_const = self.create_const_int(0);
        self.add_instruction(Instruction::Cmp(cond_status, zero_const, CmpOp::Ne, cond));
        self.add_instruction(Instruction::Branch(cond, body_block, exit_block));
    ...
    fn generate_select_statement(
        ...
        self.add_instruction(Instruction::Branch(continue_val, loop_start, loop_exit));
    ...
    fn generate_redirection(
        ...
        self.add_instruction(Instruction::Redirect(fd_val, target.to_string(), mode));
    ...
    fn generate_parameter_expansion(
        &mut self,
        parameter: &str,
        ...
        let op = match operation {
            Some(ast_op) => match ast_op {
                ...
                ParameterOperation::RemoveLargestSuffix(word) => {
                    ParamExpandOp::RemoveLargestSuffix(word)
                }
                ParameterOperation::RemovePrefix(word) => ParamExpandOp::RemovePrefix(word),
                ParameterOperation::RemoveLargestPrefix(word) => {
                    ParamExpandOp::RemoveLargestPrefix(word)
                }
            },
