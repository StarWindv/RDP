        ...
        match self {
            ...
            Instruction::ParamExpand(param, op, result) => {
                ...
                match op {
                    ...
                    ParamExpandOp::SubstringWithLength(offset, length) => {
                        write!(f, ":{}:{}", offset, length)
                    }
                    ParamExpandOp::ReplaceFirst(pattern, replacement) => {
                        write!(f, "/'{}/{}'", pattern, replacement)
                    }
                    ParamExpandOp::ReplaceAll(pattern, replacement) => {
                        write!(f, "//'{}/{}'", pattern, replacement)
                    }
                    ParamExpandOp::ReplacePrefix(pattern, replacement) => {
                        write!(f, "/#'{}/{}'", pattern, replacement)
                    }
                    ParamExpandOp::ReplaceSuffix(pattern, replacement) => {
                        write!(f, "/%'{}/{}'", pattern, replacement)
                    }
                }
            }
...
impl IrBuilder {
    ...
    pub fn create_value_with_name(&mut self, ty: ValueType, name: String) -> Option<ValueId> {
        self.current_function
            .as_mut()
            .map(|f| f.create_value_with_name(ty, name))
    }
    ...
    pub fn create_block_with_label(&mut self, label: String) -> Option<BasicBlockId> {
        self.current_function
            .as_mut()
            .map(|f| f.create_block_with_label(label))
    }
...
pub fn create_test_function(name: &str) -> Function {
    ...
    func.add_instruction(
        func.entry_block,
        Instruction::ConstString("Hello, world!".to_string(), str_val),
    );
    ...
    func.add_instruction(
        func.entry_block,
        Instruction::CallBuiltin("echo".to_string(), args, result),
    );
