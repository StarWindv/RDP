...
| 信号名     | 编号 | 核心作用说明                | Windows模拟核心API/机制                                                   | Shell实现关键要点                                                                      |
|---------|----|-----------------------|---------------------------------------------------------------------|----------------------------------------------------------------------------------|
| SIGILL  | 4  | 进程执行非法指令（如CPU不支持的指令）  | SEH结构化异常：捕获 `EXCEPTION_ILLEGAL_INSTRUCTION`                         | 建议降级：Shell不主动捕获，仅提示“不支持该信号的trap注册”；<br> 进阶：用`__try/__except`包裹执行逻辑，触发时执行`trap`命令 |
| SIGTRAP | 5  | 调试断点触发（如gdb断点）        | SEH捕获 `EXCEPTION_BREAKPOINT` / `DebugBreak()` API                   | 无需实现：Shell脚本层面几乎不用`trap`捕获，注册时提示“仅调试场景支持”                                        |
...
| SIGSEGV | 11 | 段错误（非法内存访问，如空指针）      | SEH捕获 `EXCEPTION_ACCESS_VIOLATION`                                  | 建议降级：触发时执行`trap`命令后退出，模拟POSIX默认行为                                                |
